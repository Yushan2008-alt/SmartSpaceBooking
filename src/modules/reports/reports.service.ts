...
export class ReportsService {
  ...
  async getMonthly(query: HistoryQueryDto, makerId: number) {
    ...
    let transaksi_selesai = 0;
    let transaksi_dibatalkan = 0;
    ...
    for (const tipe of TIPE_ORDER) {
      byTipe[tipe] = { tipe, total_transaksi: 0, total_jam: 0, estimasi_pendapatan: 0, realisasi_pendapatan: 0 };
    }

    for (const r of reservasis) {
      if (r.status === 'dibatalkan') {
        transaksi_dibatalkan += 1;
        continue;
      }
      total_transaksi += 1;
      ...
      if (slot) {
        slot.total_transaksi += 1;
        slot.estimasi_pendapatan += r.total_bayar;
      }
      if (r.status === 'selesai') {
        transaksi_selesai += 1;
        ...
        if (slot) {
          slot.total_jam += r.durasi_jam;
          slot.realisasi_pendapatan += r.total_bayar;
        }
      }
    }
